...
std::string yolov8_ncnn_out1_blob;
std::string yolov8_ncnn_out2_blob;
std::string yolov8_ncnn_out3_blob;
std::string yolov8_ncnn_seg_blob;
int target_size = 640;
...
void ncnn_clear() {
    yolov8_ncnn_net.clear();
}

int load(const std::string& bin, const std::string& param) {
    if (yolov8_ncnn_net.load_param(param.c_str())){
        return -1;
    }
    if (yolov8_ncnn_net.load_model(bin.c_str())){
        return -1;
    }
    return 0;
}
...
void get_blob_name(std::string in, std::string out, std::string out1, std::string out2, std::string out3, std::string seg){
    yolov8_ncnn_in_blob = in;
    yolov8_ncnn_out_blob = out;
    yolov8_ncnn_out1_blob = out1;
    yolov8_ncnn_out2_blob = out2;
    yolov8_ncnn_out3_blob = out3;
    yolov8_ncnn_seg_blob = seg;
}
...
int detect(const cv::Mat& bgr, std::vector<Object>& objects) {
    ...
    ex.input(yolov8_ncnn_in_blob.c_str(), in_pad);
    ncnn::Mat out;
